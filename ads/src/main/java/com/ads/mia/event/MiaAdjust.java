...
public class MiaAdjust {
    ...
    public static void setEventNamePurchase(String eventNamePurchase) {
        MiaAdjust.eventNamePurchase = eventNamePurchase;
    }
    ...
    static void logPaidAdImpressionValue(double revenue, String currency) {
        if (MiaAd.getInstance().getAdConfig().getAdjustConfig() != null && MiaAd.getInstance().getAdConfig().getAdjustConfig().isEnableAdjust()) {
            AdjustEvent event = new AdjustEvent(MiaAd.getInstance().getAdConfig().getAdjustConfig().getEventAdImpression());
